#!/usr/bin/env python
# input: the csv file with sample information
# the csv file contains sample ID, sample image (MRI image name) and sampl mask (MRI segmentation name)
# Note that the paths of "MRI image and MRI segmentation" and outPath are aligned
...
def main():
  ...
  # input: the csv file with sample information
  inputCSV = os.path.join(outPath, 'filter.csv')
  # output: radiomic features
  outputFilepath = os.path.join(outPath, 'filter_feature.csv')
  
  # extraction log
  progress_filename = os.path.join(outPath, 'pyrad_log.txt')
  # extraction parameter
  params = os.path.join(outPath,  'MR-3.yaml')
