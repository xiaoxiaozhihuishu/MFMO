...
### or load data for simulated test dataset
data_type = 'simu'
...
pca.fit(X_train)
train_sel_set = pca.transform(X_train)
test_sel_set = pca.transform(X_test)
...
pls.fit(X_train, label_train)
train_sel_set = pls.transform(X_train)
test_sel_set = pls.transform(X_test)
#train_sel_set = pls.inverse_transform(train_sel_set)
#test_sel_set = pls.inverse_transform(test_sel_set)

from sklearn.svm import SVC
from sklearn.pipeline import make_pipeline
regr = make_pipeline(StandardScaler(), SVC(gamma='auto'))
regr.fit(train_sel_set,label_train)
pre_train = regr.predict(train_sel_set)
pre_test = regr.predict(test_sel_set)
...
for i in range(sample_count2):
    if (pre_test[i] >= thresholds[temp_index]) & (i < test_count):
        acc_test += 1
    if (pre_test[i] < thresholds[temp_index]) & (i >= test_count):
        acc_test += 1
test_acc = acc_test/sample_count2

# te : test index including 
te = []
te.append(test_auc)
te.append(test_acc)
